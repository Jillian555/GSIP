...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='GSIP')
    ...
    parser.add_argument('--method', type=str,
                        choices=["bare", 'lwf', 'gem', 'ewc', 'mas', 'twp', 'jointtrain', 'ergnn', 'joint','Joint', 'subgraph_replay'],
    ...
    parser.add_argument('--inter_task_edges', type=strtobool, default=False,
    ...
    parser.add_argument('--ergnn_args', type=str2dict, default={'budget':[100], 'd':[0.5], 'sampler':['CM'], 'w_ll':[0.0], 'w_lg':[0.0], 'w_h':[0.0]},
    ...
    method_args = {'ergnn': args.ergnn_args, 'lwf': args.lwf_args, 'twp': args.twp_args, 'ewc': args.ewc_args,
                   'bare': args.bare_args, 'gem': args.gem_args, 'mas': args.mas_args, 'subgraph_replay': args.sgreplay_args, 'joint': args.joint_args}
    ...
    AP_best, hyp_best_str, name_best = 0, None, None
    ...
    for hyp_params in hyp_param_list:
        ...
        name = f'{subfolder}val_{args.dataset}_{args.n_base}_{args.n_cls_per_task}_{args.method}_{list(hyp_params.values())}_{args.w_kd}'
        if args.minibatch:
        ...
        if os.path.isfile(f'{args.result_path}/{name}.pkl') and not args.overwrite_result:
            ...
            for ite in range(args.repeats):
                ...
                try:
                    ...
                    if ite > 0:
                        name_ = f'{subfolder}val_{args.dataset}_{args.n_base}_{args.n_cls_per_task}_{args.method}_{list(hyp_params.values())}_{args.w_kd}'
